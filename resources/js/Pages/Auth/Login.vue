<template>
    <div class="min-w-screen min-h-screen bg-green-100 flex items-center justify-center px-5 py-5">
        <div class="bg-gray-300 text-gray-500 rounded-3xl shadow-xl w-full overflow-hidden" style="max-width:1000px">
            <div class="md:flex w-full">
                <div class="hidden md:block w-1/2 bg-green-300 py-10 px-10">
            ...
            <div class="relative">
                <jet-label for="email" class="text-lg text-center" value="Email" />
                <jet-input id="email" type="email" class="text-black pl-8 border-b-2 font-display focus:outline-none focus:border-primarycolor transition-all duration-500 text-lg" v-model="form.email" required autofocus />
            </div>

            <div class="relative mt-6">
                <jet-label for="password" class="text-lg text-center" value="Password" />
                <jet-input id="password" type="password" class="text-black pl-8 border-b-2 font-display focus:outline-none focus:border-primarycolor transition-all duration-500 text-lg" v-model="form.password" required autocomplete="current-password" />
            </div>

            <div class="block mt-4">
                <label class="flex items-center">
                    <jet-checkbox name="remember" v-model="form.remember" />
                    <span class="ml-2 text-sm text-gray-800 font-bold">Remember me</span>
                </label>
            </div>
            </div>

            <div class="text-center">
			<jet-button class="py-3 px-20 bg-themecolor rounded-full text-white font-bold uppercase text-md mt-4 transform hover:translate-y-1 transition-all duration-500" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">
                    Login
            </jet-button>
            </div>
            <div class="text-center text-black mt-5">
                <inertia-link v-if="canResetPassword" :href="route('password.request')" class="mx-auto text-gray-600">
                    Forgot password?
                </inertia-link>
                <span class="font-bold">·</span>
                <inertia-link :href="route('register')" class="mx-auto text-gray-600">
                    Register an account
                </inertia-link>
                            </div>
                        </form>
